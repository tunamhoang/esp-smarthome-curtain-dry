...
#define ESP_WS_CLIENT_MEM_CHECK(TAG, a, action) if (!(a)) {                                         \
        LOGE(TAG, "%s(%d): %s", __FUNCTION__, __LINE__, "Memory exhausted");                             \
        action;                                                                                     \
        }
...
#define ESP_WS_CLIENT_ERR_OK_CHECK(TAG, err, action)  { \
        ...
        if (_esp_ws_err_to_check != ESP_OK) {           \
            LOGE(TAG, "%s(%d): Expected ESP_OK; reported: %d", __FUNCTION__, __LINE__, _esp_ws_err_to_check); \
            action;                                     \
            }                                           \
        }

#define ESP_WS_CLIENT_STATE_CHECK(TAG, a, action) if ((a->state) < WEBSOCKET_STATE_INIT) {                                         \
        LOGE(TAG, "%s(%d): %s", __FUNCTION__, __LINE__, "Websocket already stop");                       \
        action;                                                                                     \
        }
...
static esp_err_t websocket_new_buf(websocket_client_handle_t client, bool is_tx) {
#ifdef CONFIG_ESP_WS_CLIENT_ENABLE_DYNAMIC_BUFFER
    if (is_tx) {
        if (client->tx_buffer) free(client->tx_buffer);
        client->tx_buffer = calloc(1, client->buffer_size);
        ESP_WS_CLIENT_MEM_CHECK(TAG, client->tx_buffer, return ESP_ERR_NO_MEM);
    } else {
        if (client->rx_buffer) free(client->rx_buffer);
...
static void websocket_free_buf(websocket_client_handle_t client, bool is_tx) {
#ifdef CONFIG_ESP_WS_CLIENT_ENABLE_DYNAMIC_BUFFER
    if (is_tx) {
        if (client->tx_buffer) {
...
static esp_err_t websocket_client_dispatch_event(websocket_client_handle_t client,
        websocket_event_id_t event,
        const char *data,
        int data_len) {
    esp_err_t err;
    websocket_event_data_t event_data;
...
static esp_err_t websocket_client_abort_connection(websocket_client_handle_t client) {
    ESP_WS_CLIENT_STATE_CHECK(TAG, client, return ESP_FAIL);
    esp_transport_close(client->transport);
    if (client->config->auto_reconnect) {
        client->reconnect_tick_ms = _tick_get_ms();
        LOGI(TAG, "Reconnect after %d ms", client->wait_timeout_ms);
    }
    client->state = WEBSOCKET_STATE_WAIT_TIMEOUT;
    websocket_client_dispatch_event(client, WEBSOCKET_EVENT_DISCONNECTED, NULL, 0);
    return ESP_OK;
}

static esp_err_t websocket_client_error_connection(websocket_client_handle_t client) {
    ESP_WS_CLIENT_STATE_CHECK(TAG, client, return ESP_FAIL);
    esp_transport_close(client->transport);
    if (client->config->auto_reconnect) {
        client->reconnect_tick_ms = _tick_get_ms();
    ...
    client->state = WEBSOCKET_STATE_WAIT_TIMEOUT;
    websocket_client_dispatch_event(client, WEBSOCKET_EVENT_ERROR, NULL, 0);
    return ESP_OK;
}

static esp_err_t websocket_client_set_config(websocket_client_handle_t client, const websocket_client_config_t *config) {
    websocket_config_storage_t *cfg = client->config;
    cfg->task_prio = config->task_prio;
    if (cfg->task_prio <= 0) cfg->task_prio = WEBSOCKET_TASK_PRIORITY;
    cfg->task_stack = config->task_stack;
    if (cfg->task_stack == 0) cfg->task_stack = WEBSOCKET_TASK_STACK;
    if (config->host) {
    ...
    if (config->disable_auto_reconnect) cfg->auto_reconnect = false;
    if (config->disable_pingpong_discon) cfg->pingpong_timeout_sec = 0;
    else if (config->pingpong_timeout_sec) cfg->pingpong_timeout_sec = config->pingpong_timeout_sec;
    else cfg->pingpong_timeout_sec = WEBSOCKET_PINGPONG_TIMEOUT_SEC;
    if (config->network_timeout_ms <= 0) {
        cfg->network_timeout_ms = WEBSOCKET_NETWORK_TIMEOUT_MS;
        LOGW(TAG, "`network_timeout_ms` is not set, or it is less than or equal to zero, using default time out %d (milliseconds)", WEBSOCKET_NETWORK_TIMEOUT_MS);
    }
    else cfg->network_timeout_ms = config->network_timeout_ms;
    if (config->ping_interval_sec == 0) cfg->ping_interval_sec = WEBSOCKET_PING_INTERVAL_SEC;
    else cfg->ping_interval_sec = config->ping_interval_sec;
    return ESP_OK;
}

static esp_err_t websocket_client_destroy_config(websocket_client_handle_t client) {
    if (client == NULL) return ESP_ERR_INVALID_ARG;
    websocket_config_storage_t *cfg = client->config;
    if (client->config == NULL) return ESP_ERR_INVALID_ARG;
    free(cfg->host);
    free(cfg->uri);
    free(cfg->path);
    free(cfg->scheme);
...
static esp_err_t set_websocket_transport_optional_settings(websocket_client_handle_t client, const char *scheme) {
...
static esp_err_t websocket_client_create_transport(websocket_client_handle_t client) {
    if (!client->config->scheme) {
        LOGE(TAG, "No scheme found");
        return ESP_FAIL;
    }
    ...
    if (strcasecmp(client->config->scheme, WS_OVER_TCP_SCHEME) == 0) {
        ...
        if (client->config->crt_bundle_attach) {
            ...
            LOGE(TAG, "crt_bundle_attach configured but not enabled in menuconfig: Please enable MBEDTLS_CERTIFICATE_BUNDLE option");
#endif
        }
        ...
        LOGE(TAG, "Not support this websocket scheme %s, only support %s and %s", client->config->scheme, WS_OVER_TCP_SCHEME, WS_OVER_TLS_SCHEME);
        return ESP_FAIL;
    }
    return ESP_OK;
}
...
websocket_client_handle_t websocket_client_init(const websocket_client_config_t *config) {
    websocket_client_handle_t client = calloc(1, sizeof(struct websocket_client));
    ...
    if (esp_event_loop_create(&event_args, &client->event_handle) != ESP_OK) {
        LOGE(TAG, "Error create event handler for websocket client");
        free(client);
        return NULL;
    }
    ...
    if (config->reconnect_timeout_ms <= 0) {
        ...
        LOGW(TAG, "`reconnect_timeout_ms` is not set, or it is less than or equal to zero, using default time out %d (milliseconds)", WEBSOCKET_RECONNECT_TIMEOUT_MS);
    } else {
    ...
    if (config->uri) {
        if (websocket_client_set_uri(client, config->uri) != ESP_OK) {
            LOGE(TAG, "Invalid uri");
            goto _websocket_init_fail;
        }
    }
    if (websocket_client_set_config(client, config) != ESP_OK) {
        LOGE(TAG, "Failed to set the configuration");
        goto _websocket_init_fail;
    }
    ...
    if (buffer_size <= 0) buffer_size = WEBSOCKET_BUFFER_SIZE_BYTE;
#ifndef CONFIG_ESP_WS_CLIENT_ENABLE_DYNAMIC_BUFFER
    client->rx_buffer = malloc(buffer_size);
    ESP_WS_CLIENT_MEM_CHECK(TAG, client->rx_buffer, goto _websocket_init_fail);
    client->tx_buffer = malloc(buffer_size);
    ESP_WS_CLIENT_MEM_CHECK(TAG, client->tx_buffer, goto _websocket_init_fail);
#endif
    client->status_bits = xEventGroupCreate();
    ESP_WS_CLIENT_MEM_CHECK(TAG, client->status_bits, goto _websocket_init_fail);
...
esp_err_t websocket_client_destroy(websocket_client_handle_t client) {
    if (client == NULL) return ESP_ERR_INVALID_ARG;
    if (client->run) websocket_client_stop(client);
    if (client->event_handle) esp_event_loop_delete(client->event_handle);
    if (client->if_name) free(client->if_name);
    websocket_client_destroy_config(client);
    esp_transport_list_destroy(client->transport_list);
...
esp_err_t websocket_client_set_uri(websocket_client_handle_t client, const char *uri) {
    if (client == NULL || uri == NULL) return ESP_ERR_INVALID_ARG;
    struct http_parser_url puri;
    http_parser_url_init(&puri);
    int parser_status = http_parser_parse_url(uri, strlen(uri), 0, &puri);
    if (parser_status != 0) {
        LOGE(TAG, "Error parse uri = %s", uri);
        return ESP_FAIL;
    }
...
static esp_err_t websocket_client_recv(websocket_client_handle_t client) {
    int rlen;
    client->payload_offset = 0;
    CHECK_RETURN(websocket_new_buf(client, false) != ESP_OK, ESP_FAIL, LOGE(TAG, "Failed to setup rx buffer"));
    do {
        ...
        if (rlen < 0) {
            ...
            websocket_free_buf(client, false);
            return ESP_FAIL;
        }
        ...
        if (rlen == 0 && client->last_opcode == WS_TRANSPORT_OPCODES_NONE ) {
            LOGW(TAG, "esp_transport_read timeouts");
            websocket_free_buf(client, false);
            return ESP_OK;
        }
        websocket_client_dispatch_event(client, WEBSOCKET_EVENT_DATA, client->rx_buffer, rlen);
    ...
    if (client->last_opcode == WS_TRANSPORT_OPCODES_PING) {
        ...
        LOGD(TAG, "Sending PONG with payload len=%d", client->payload_len);
...
static void websocket_client_task(void *pv) {
    ...
    if (client->transport == NULL) {
        LOGE(TAG, "There are no transports valid, stop websocket client");
        client->run = false;
    }
    //default port
    ...
    while (client->run) {
        if (xSemaphoreTakeRecursive(client->lock, lock_timeout) != pdPASS) {
            LOGE(TAG, "Failed to lock ws-client tasks, exiting the task...");
            break;
        }
        switch ((int)client->state) {
        case WEBSOCKET_STATE_INIT:
            if (client->transport == NULL) {
                LOGE(TAG, "There are no transport");
                client->run = false;
                break;
            }
            int result = esp_transport_connect(client->transport,
                                               ...
                                               client->config->network_timeout_ms);
            if (result < 0) {
                LOGE(TAG, "Error transport connect %i", result);
                websocket_client_error_connection(client);
                break;
            }
            LOGD(TAG, "Transport connected to %s://%s:%d", client->config->scheme, client->config->host, client->config->port);
            client->state = WEBSOCKET_STATE_CONNECTED;
            client->wait_for_pong_resp = false;
            websocket_client_dispatch_event(client, WEBSOCKET_EVENT_CONNECTED, NULL, 0);
            break;
        case WEBSOCKET_STATE_CONNECTED:
            if ((CLOSE_FRAME_SENT_BIT & xEventGroupGetBits(client->status_bits)) == 0) { // only send and check for PING
                ...
                if ( _tick_get_ms() - client->pingpong_tick_ms > client->config->pingpong_timeout_sec * 1000 ) {
                    if (client->wait_for_pong_resp) {
                        LOGE(TAG, "Error, no PONG received for more than %d seconds after PING", client->config->pingpong_timeout_sec);
            ...
            if (read_select == 0) {
                LOGD(TAG, "Read poll timeout: skipping esp_transport_read()...");
                break;
            }
            client->ping_tick_ms = _tick_get_ms();
            if (websocket_client_recv(client) == ESP_FAIL) {
                LOGE(TAG, "Error receive data");
                websocket_client_abort_connection(client);
                break;
            }
            break;
        ...
        case WEBSOCKET_STATE_CLOSING:
            ...
            if ((CLOSE_FRAME_SENT_BIT & xEventGroupGetBits(client->status_bits)) == 0) {
                LOGD(TAG, "Closing initiated by the server, sending close frame");
            ...
            break;
        default:
            LOGD(TAG, "Client run iteration in a default state: %d", client->state);
            break;
        }
        ...
        if (WEBSOCKET_STATE_CONNECTED == client->state) {
            ...
            if (read_select < 0) {
                LOGE(TAG, "Network error: esp_transport_poll_read() returned %d, errno=%d", read_select, errno);
                websocket_client_abort_connection(client);
            }
            ...
            LOGD(TAG, "Waiting for TCP connection to be closed by the server");
            ...
            if (ret < 0) {
                LOGW(TAG, "Connection terminated while waiting for clean TCP close");
            }
            client->run = false;
            client->state = WEBSOCKET_STATE_UNKNOW;
            websocket_client_dispatch_event(client, WEBSOCKET_EVENT_CLOSED, NULL, 0);
            break;
        }
    }
    LOGW(TAG, "Task destroy...");
    esp_transport_close(client->transport);
...
esp_err_t websocket_client_start(websocket_client_handle_t client) {
    if (client == NULL) return ESP_ERR_INVALID_ARG;
    CHECK_RETURN(client->state >= WEBSOCKET_STATE_INIT, ESP_FAIL, LOGW(TAG, "The client has started"));
    CHECK_RETURN(websocket_client_create_transport(client) != ESP_OK, ESP_FAIL, LOGW(TAG, "Failed to create websocket transport"));
    if (xTaskCreate(websocket_client_task, "websocket_task", client->config->task_stack, client, client->config->task_prio, &client->task_handle) != pdTRUE) {
        LOGE(TAG, "Error create websocket task");
        return ESP_FAIL;
    }
    xEventGroupClearBits(client->status_bits, STOPPED_BIT | CLOSE_FRAME_SENT_BIT);
    return ESP_OK;
...
esp_err_t websocket_client_stop(websocket_client_handle_t client) {
    if (client == NULL) return ESP_ERR_INVALID_ARG;
    CHECK_RETURN(!client->run, ESP_FAIL, LOGW(TAG, "Client was not started"));
    /* A running client cannot be stopped from the websocket task/event handler */
    TaskHandle_t running_task = xTaskGetCurrentTaskHandle();
    CHECK_RETURN(running_task == client->task_handle, ESP_FAIL, LOGW(TAG, "Client cannot be stopped from websocket task"));
    client->run = false;
...
static esp_err_t websocket_client_close_with_optional_body(websocket_client_handle_t client, bool send_body, int code, const char *data, int len, TickType_t timeout) {
    if (client == NULL) return ESP_ERR_INVALID_ARG;
    CHECK_RETURN(!client->run, ESP_FAIL, LOGW(TAG, "Client was not started"));
    /* A running client cannot be stopped from the websocket task/event handler */
    TaskHandle_t running_task = xTaskGetCurrentTaskHandle();
    ...
    xEventGroupSetBits(client->status_bits, CLOSE_FRAME_SENT_BIT);
    if (STOPPED_BIT & xEventGroupWaitBits(client->status_bits, STOPPED_BIT, false, true, timeout)) return ESP_OK;
...
int websocket_client_send_with_opcode(websocket_client_handle_t client, ws_transport_opcodes_t opcode, const uint8_t *data, int len, TickType_t timeout) {
    ...
    int wlen = 0, widx = 0;
    int ret = ESP_FAIL;
    CHECK_RETURN(client == NULL || len < 0 || (opcode != WS_TRANSPORT_OPCODES_CLOSE && (data == NULL || len <= 0)),
                 ESP_FAIL, LOGW(TAG, "Invalid arguments"));
    CHECK_RETURN(xSemaphoreTakeRecursive(client->lock, timeout) != pdPASS, ESP_FAIL, LOGE(TAG, "Could not lock ws-client within %ld timeout", timeout));
    CHECK_GOTO(!websocket_client_is_connected(client), unlock_and_return, LOGE(TAG, "Websocket client is not connected"));
    CHECK_GOTO(client->transport == NULL, unlock_and_return, LOGE(TAG, "Invalid transport"));
    CHECK_GOTO(websocket_new_buf(client, true) != ESP_OK, unlock_and_return, LOGE(TAG, "Failed to setup tx buffer"));
    uint32_t current_opcode = opcode;
    while (widx < len || current_opcode) {  // allow for sending "current_opcode" only message with len==0
        if (need_write > client->buffer_size) need_write = client->buffer_size;
        else current_opcode |= WS_TRANSPORT_OPCODES_FIN;
        ...
        if (wlen < 0 || (wlen == 0 && need_write != 0)) {
            ret = wlen;
            LOGE(TAG, "Network error: esp_transport_write() returned %d, errno=%d", ret, errno);
            websocket_free_buf(client, true);
            websocket_client_abort_connection(client);
...
size_t websocket_client_get_ping_interval_sec(websocket_client_handle_t client) {
    CHECK_RETURN(client == NULL, 0, LOGW(TAG, "Client was not initialized"));
    CHECK_RETURN(client->config == NULL, 0, LOGW(TAG, "No config available to change the ping interval"));
...
esp_err_t websocket_client_set_ping_interval_sec(websocket_client_handle_t client, size_t ping_interval_sec) {
    CHECK_RETURN(client == NULL, ESP_ERR_INVALID_ARG, LOGW(TAG, "Client was not initialized"));
    CHECK_RETURN(client->config == NULL, ESP_ERR_INVALID_STATE, LOGW(TAG, "No config available to change the ping interval"));

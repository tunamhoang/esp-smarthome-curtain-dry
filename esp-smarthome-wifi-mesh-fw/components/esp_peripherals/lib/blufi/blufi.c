...
void blufi_dh_negotiate_data_handler(uint8_t *data, int len, uint8_t **output_data, int *output_len, bool *need_free) {
    ...
    uint8_t type = data[0];
    if (blufi_sec == NULL) {
        LOGE(TAG, "BLUFI Security is not initialized");
    ...
    switch (type) {
        case SEC_TYPE_DH_PARAM_LEN:
            ...
            if (blufi_sec->dh_param_len > MAX_DH_PARAM_LEN) {
                btc_blufi_report_error(ESP_BLUFI_DH_MALLOC_ERROR);
                LOGE(TAG, "%s, MAX_DH_PARAM_LEN = %d\n", __func__, blufi_sec->dh_param_len);
                return;
            }
        ...
        case SEC_TYPE_DH_PARAM_DATA: {
                if (blufi_sec->dh_param == NULL) {
                    LOGE(TAG, "%s, blufi_sec->dh_param == NULL\n", __func__);
                ...
                if (ret) {
                    LOGE(TAG, "%s read param failed %d\n", __func__, ret);
                ...
                if (ret) {
                    LOGE(TAG, "%s make public failed %d\n", __func__, ret);
                ...
                ret = mbedtls_dhm_calc_secret(&blufi_sec->dhm,
                                        ...
                                        myrand, NULL);
                if (ret) {
                    LOGE(TAG, "%s mbedtls_dhm_calc_secret failed %d\n", __func__, ret);
                    btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
                    return;
                }
                ret = mbedtls_md5(blufi_sec->share_key, blufi_sec->share_len, blufi_sec->psk);
                if (ret) {
                    LOGE(TAG, "%s mbedtls_md5 failed %d\n", __func__, ret);

...
static tct_arguments* tct_find_arguments(tct_arguments *arguments, const char *name, size_t name_len) {
    while(arguments)
...
static char* tct_get_valuen(tct_arguments *arguments, char *name, size_t name_len) {
...
char *tct_render_uint64(const char *template, const char *key, const char *format, uint64_t value)
{
    ...
    tct_add_argument(args, key, format, value);
...
{
    tct_arguments *args = NULL;
    tct_add_argument(args, "id", "%06llX", (value >> 8) & 0xFFFFFF);
    tct_add_argument(args, "xid", "%08llX", value & 0xFFFFFFFF);
    char *render = tct_render((char *)template, args);
    tct_free_argument(args);
    return render;
}
